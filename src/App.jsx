...
export default function App() {
  ...
  function saveCarryover(k) {
    updateKey(k, { carryover: Math.max(0, parseInt(coInput) || 0), coSet: true });
    setEditCO(null);
  }
  ...
  function saveInitRem(k, person, yr) {
    ...
    const initUsed = Math.max(0, eff - entered - booked);
    updateKey(k, { initUsed, initSet: true });
    setEditRem(null);
  }
  ...
  function addHoliday() {
    ...
    if (days === 0) return setFormError("No working days in that range.");
  ...
  function removeHoliday(pid, yr, id) {
    updateKey(sKey(pid, yr), { holidays: D(pid, yr).holidays.filter(h => h.id !== id) });
  }

  function card(person, baseYear, isNext) {
    const k = sKey(person.id, baseYear);
    const d = D(person.id, baseYear);
    const eff = effAllowance(person, baseYear);
    const u = usedDays(person.id, baseYear);
    const rem = remDays(person, baseYear);
      ...
      <div key={k} style={{ background:"white", borderRadius:16, padding:20, boxShadow:"0 1px 4px rgba(0,0,0,0.07)", borderTop:`4px solid ${person.color}` }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", margin:"10px 0 12px" }}>
          <div>
            ...
            {editRem === k ? (
              ...
              <div style={{ display:"flex", alignItems:"baseline", gap:8 }}>
                ...
                <button onClick={() => { setEditRem(k); setRemInput(rem); setEditCO(null); }}
                  title="Set initial remaining days"
          ...
          <div style={{ textAlign:"right" }}>
            ...
            <div style={{ fontSize:11, color:"#94a3b8", marginTop:1 }}>Base: {person.allowance}{d.initUsed > 0 ? ` · Pre-loaded: ${d.initUsed}` : ""}</div>
          </div>
        </div>
        ...
        {hols.map(h => (
          <div key={h.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 12px", borderRadius:8, background:bgColor, marginBottom:6 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:"#64748b" }}>{fmtDate(h.start)} – {fmtDate(h.end)} · <strong>{h.days}</strong> day{h.days!==1?"s":""}</div>
            </div>
  ...
  function holidayPeopleOn(ds) {
    return PEOPLE.filter(p => Object.keys(data).some(k => k.startsWith(p.id+"-") && (data[k].holidays||[]).some(h => ds >= h.start && ds <= h.end)));
  }
          ...
          <div style={{ display:"flex", gap:4 }}>
            {["dashboard","calendar","add"].map(v => (
              <button key={v} onClick={() => setView(v)} style={{
                padding:"8px 18px", border:"none", cursor:"pointer", borderRadius:"8px 8px 0 0",
                background: view===v ? "white" : "rgba(255,255,255,0.15)",
                color: view===v ? "#6366f1" : "white", fontWeight:600, fontSize:13
              }}>{v==="add"?"+ Add Holiday":v.charAt(0).toUpperCase()+v.slice(1)}</button>
            ))}
          </div>
        </div>
      </div>
        ...
        {view === "dashboard" && (
          <div>
            <div style={{ marginBottom:28 }}>
              ...
              {mkYears.every(yr => D("MK", yr).holidays.length === 0) &&
                <p style={{ margin:0, color:"#94a3b8", fontSize:13 }}>No holidays added yet for MK.</p>}
            </div>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              ...
              {D("SL", slCurYear).holidays.length === 0 &&
                <p style={{ margin:0, color:"#94a3b8", fontSize:13 }}>No holidays added yet for SL.</p>}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ background:"white", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)" }}>
                {Array.from({length:getFirstDay(calYear,calMonth)}).map((_,i)=><div key={`e${i}`} style={{minHeight:60,borderTop:"1px solid #f1f5f9"}}/>)}
                {Array.from({length:getDaysInMonth(calYear,calMonth)}).map((_,i)=>{
                  const d=i+1, ds=`${calYear}-${String(calMonth+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
                  const isToday=ds===todayStr, weekend=isWeekend(ds), people=holidayPeopleOn(ds);
                  return (
                    <div key={d} style={{minHeight:60,borderTop:"1px solid #f1f5f9",padding:4,background:isToday?"#fffbeb":weekend?"#fafafa":"white"}}>
                      <div style={{fontSize:12,fontWeight:isToday?800:400,color:isToday?"#f59e0b":weekend?"#cbd5e1":"#334155",marginBottom:2}}>{d}</div>
        ...
        {view === "add" && (
          <div style={{ background:"white", borderRadius:16, padding:28, boxShadow:"0 1px 4px rgba(0,0,0,0.07)", maxWidth:480 }}>
            ...
            {form.start && form.end && form.end >= form.start && (() => {
              const days = countWorkdays(form.start, form.end);
              const baseYear = getBaseYear(formPerson, new Date(form.start + "T12:00:00"));
              const rem = remDays(formPerson, baseYear);
              return (
                <div style={{ background:days>rem?"#fef2f2":"#f0fdf4", border:`1px solid ${days>rem?"#fecaca":"#bbf7d0"}`, borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:13, color:days>rem?"#b91c1c":"#166534" }}>
                  📅 <strong>{days}</strong> working day{days!==1?"s":""} · <strong>{Math.max(0,rem-days)}</strong> remaining after
                </div>
              );
            })()}
